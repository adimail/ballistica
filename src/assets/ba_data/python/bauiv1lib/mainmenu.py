...
from bauiv1lib.rlmapselect import RLMapSelectWindow
...
class MainMenuWindow(bui.MainWindow):
    ...
    def _run_training_press(self) -> None:
        if not self.main_window_has_control():
            return
        self.main_window_replace(RLMapSelectWindow(origin_widget=self._training_button))
    ...
    def _refresh(self) -> None:
        ...
        if bui.app.env.demo or bui.app.env.arcade:
            ...
            self._demo_menu_button = bui.buttonwidget(
                ...
                label=bui.Lstr(
                    resource=(
                        "modeArcadeText" if bui.app.env.arcade else "modeDemoText"
                    )
                ),
        ...
        # Example: Adding it below the main row of buttons
        # Adjust 'v' from where your last row of buttons ended.
        # This is a very rough placement.
        v_training_button = button_y_offs - 80  # Adjust this offset
        h_training_button = self._width * 0.5

        self._training_button = bui.buttonwidget(
            parent=self._root_widget,
            position=(
                h_training_button - self._button_width * 0.5 * 0.8,
                v_training_button,
            ),  # Example pos
            size=(
                self._button_width * 0.8,
    ...
    def _restore_state(self) -> None:
        try:
            ...
            if sel_name == "Training":
                sel = self._training_button
            if sel_name is None:
                sel_name = "Start"
